...
export default function SegmentsPage() {
  ...
  const [query, setQuery] = useQueryState<{ builder?: string }>({ builder: undefined });

  useEffect(() => {
    if (!isClient) return;
    if (!query.builder) return;
    try {
      const decoded = JSON.parse(decodeURIComponent(query.builder)) as BuilderGroup;
      setStoredGroup(decoded);
    } catch (error) {
      console.warn("Failed to parse builder state", error);
    }
  }, [isClient, query.builder]);

  useEffect(() => {
    if (!isClient) return;
    const encoded = encodeURIComponent(JSON.stringify(storedGroup));
    if (query.builder !== encoded) {
      setQuery({ builder: encoded });
    }
  }, [isClient, query.builder, setQuery, storedGroup]);
