...
import { copyDeepLink, useQueryState } from "@/lib/url-state";
...
export default function RadarPage() {
  ...
  const [query, setQuery] = useQueryState<{ geo?: string; product?: string; segment?: string; from?: string; to?: string }>({
    geo: undefined,
    product: undefined,
    segment: undefined,
    from: undefined,
  ...
  const geo = toSingle(query.geo);
  const product = toSingle(query.product);
  const segment = toSingle(query.segment);
  const dateFrom = toSingle(query.from);
  const dateTo = toSingle(query.to);
  ...
  const handleOpenStudio = (opportunity: RankedOpportunity) => {
    ...
    params.set("seedId", seedId);
    router.push(`/segments${params.toString() ? `?${params.toString()}` : ""}`);
  };
  ...
  return (
    <main className="space-y-8 p-6">
      <header className="space-y-1">
        <h1 className="text-xl font-semibold text-slate-900">Opportunity Radar</h1>
      ...
      <section className="sticky top-16 z-10 space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          <div>
            ...
            <select
              value={geo ?? ""}
              onChange={(event) => setQuery({ geo: event.target.value || undefined })}
          ...
          <div>
            ...
            <select
              value={product ?? ""}
              onChange={(event) => setQuery({ product: event.target.value || undefined })}
          ...
          <div>
            ...
            <select
              value={segment ?? ""}
              onChange={(event) => setQuery({ segment: event.target.value || undefined })}
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <select
                value={dateFrom ?? ""}
                onChange={(event) => setQuery({ from: event.target.value || undefined })}
              ...
              <select
                value={dateTo ?? ""}
                onChange={(event) => setQuery({ to: event.target.value || undefined })}
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-300"
              >
