...
function RankedOpportunityCard({ opportunity, rank, minAudience, onOpenStudio }: RankedOpportunityCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        <Freshness updatedAt={opportunity.updatedAt} slaHours={opportunity.slaHours} lineage={opportunity.lineage} />
        <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-600">
          {opportunity.geography} · {opportunity.product} · {opportunity.segmentTag}
        </span>
      </div>
      ...
      <div className="mt-4">
        <ScenarioStrip audience={opportunity.audience} scenario={opportunity.scenario} />
      </div>
      ...
      <div className="mt-4 border-t border-slate-200 pt-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Segment Studio</p>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={onOpenStudio}
            className="rounded-lg border border-brand-300 px-3 py-1 text-xs font-semibold text-brand-700 transition hover:border-brand-500 hover:text-brand-800"
          >
            View in Segment Studio
          </button>
          <span className="text-xs text-slate-500">Preserves filters and drivers for deep-dive.</span>
        </div>
      </div>
    </article>
  );
