...
<html lang="en">
  ...
  const handleOpenBrief = () => {
    logAudit({
      type: "execution.export.brief",
      timestamp: new Date().toISOString(),
      route: "/execution",
      payload: { campaignId: selectedCampaignId },
    });
    window.open("/execution/brief", "_blank", "noopener");
  };
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-semibold uppercase tracking-wide text-slate-600">SLA timer</th>
                ...
                <tr key={task.id} className="hover:bg-brand/5">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    <div className="flex items-center gap-2">
                      <SLATimer dueDate={task.dueDate} />
                      <span className="text-xs text-slate-400">{task.slaHours}h SLA</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-600">
